...
export default function ShowEmployees({ addEmployee }) {
    ...
    return (
        <main className="flex-1 bg-zinc-50 dark:bg-zinc-900 min-h-screen p-8">
                        ...
                        {filteredEmpolyees && filteredEmpolyees.map((employee) => (
                            <tr key={employee.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-700/30 transition-colors duration-200 group">
                                ...
                                <td className="px-6 py-4 text-right ">
                                    <button onClick={() => setSelectedEmployee(employee)} className="p-2 hover:bg-zinc-200 dark:hover:bg-zinc-600 rounded-lg transition-colors relative overflow-visible">
            ...
            {
                selectedEmployee && <Actions user={selectedEmployee} closeActions={() => setSelectedEmployee(null)} />
            }
        </main>
    )
}
